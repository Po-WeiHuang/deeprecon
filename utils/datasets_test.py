...
from uprootdataset_test import UprootMultiFileDataset
    ...
    def __iter__(self) -> Iterator[tuple[dict[str, torch.Tensor], dict[str, torch.Tensor]]]:
        ...
        for item in super().__iter__():
            ...
            truth = {
                ...
                "evtime": torch.tensor(float(record["mctime1"]), dtype=torch.float32),
                "multisite": torch.tensor(multisite_label, dtype=torch.float32)
            }
...
def main():
    # Configuration
    solar_path = "/data/snoplus2/weiiiii/electron_bismsb_0dot5_10/Ntuple/Bronze/37121*.ntuple.root"
    multisite_path = "/data/snoplus2/weiiiii/electron_bismsb_0dot5_10/Ntuple/Bronze/37111*.ntuple.root"
    truth_vars = ["mcPosx", "mcPosy", "mcPosz", "mcPosr", "mcEdepQuenched","mctime1"]

    # 1. Initialize Dataset
    dataset = PosEnergyRecoDataset(
        solar_paths=solar_path,
        multisite_paths=multisite_path,
        tree_name="output",
    ...
    print(f"{'Batch':<6} | {'Event lengths in batch':<30} | {'Avg Energy'}| {'Multisite'}")
    print("-" * 65)
    ...
    for i, (x_packed, truth, cu_seq, max_len) in enumerate(dataloader):
        ...
        avg_energy = torch.mean(truth["energy"]).item()
        multisite = torch.mean(truth["multisite"]).item()
        print(f"{i:<6} | {str(len(total_hits)):<30} | {avg_energy:<8.2f} MeV | {multisite:<8.2f}")
        #print(f"Batch {i}")
